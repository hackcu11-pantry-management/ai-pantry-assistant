...
const ExamplePage = () => {
  ...
  const renderProductInfo = (item) => {
    return (
      <div key={item.productupc || item.upc} className="border rounded-lg p-4 mb-4 shadow-sm">
        <h2 className="text-xl font-semibold mb-2">{item.productname || item.title}</h2>
        {(item.productimages || item.images) && (item.productimages || item.images).length > 0 && (
          <img 
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="col-span-2">{item.productdescription || item.description || 'No description available'}</div>
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {result && result.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold mb-3">Found {result.length} item(s):</h2>
